...
using split put the cards in two arrays
for the interwoven we could use the function zip and then flatten the tupples
  or we could just traverse the arrays and place the cards in a third array
...
'''
def faro(deck):
  halfCount = len(deck)//2
  firstHalf = deck[:halfCount]
  secondHalf = deck[halfCount:]
...
def faroUsingZip(deck):

  halfCount = len(deck)//2
  firstHalf = deck[:halfCount]
  secondHalf = deck[halfCount:]

  newShuffle = zip(firstHalf, secondHalf)

  return [y for x in newShuffle for y in x]
